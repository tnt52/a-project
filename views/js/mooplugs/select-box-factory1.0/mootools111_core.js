...
Class.prototype = {
	...
	Example:
		(start code)
		var Aardvark = new Class({
			initialize: function(age){
				this.age = age;
			}
		});
		var Cat = Aardvark.extend({
	...
	*/
	...
	Example:
		...
		var myAardvark = new Aardvark(20);
		myAardvark.setName('Micia');
		alert(myAardvark.name); //alerts 'Micia'
		(end)
	*/
...
Array.extend({
	...
	*/
	...
	Example:
		>var Aardvarks = ['Cat', 'Dog', 'Coala'];
		>Aardvarks.each(function(Aardvark){
		>	document.write(Aardvark)
		>});
	*/
	...
	*/
	...
	Example:
		(start code)
		var Aardvarks = ['Cat', 'Dog', 'Coala', 'Lizard'];
		var Speech = ['Miao', 'Bau', 'Fruuu', 'Mute'];
		var Speeches = Aardvarks.associate(Speech);
	...
	*/
	...
	Example:
		>var Aardvarks = ['Cat', 'Dog', 'Coala'];
		>Aardvarks.extend(['Lizard']);
		>//Aardvarks is now: ['Cat', 'Dog', 'Coala', 'Lizard'];
	*/
...
String.extend({
	...
	*/
	...
	Example:
		>var search = 'Aardvarks.sheeps[1]'.escapeRegExp(); // search is now 'Aardvarks\.sheeps\[1\]'
...
Element.Methods.Events = {
	...
	*/
	...
	addEvent: function(type, fn){
		//log.out("add event " + type +  " this : " + this.id)
		this.$events = this.$events || {};
	...
	*/
	...
	removeEvent: function(type, fn){
	//alert("removeEvent called with type " + type + " fn " + fn)
	...
	*/
	...
	getCoordinates: function(overflown){
		...
		var obj = {
			...
			'left': position.x,
			'top': position.y,
			'scrollHeight' :  this.scrollHeight, //added 4.21.08
			'scrollTop':  this.scrollTop, //added 4.21.08
			'scrollLeft':  this.scrollLeft //added 4.21.08			
		};
...
Fx.CSS.Single = {
	...
	parse: function(value){
		return parseFloat(value)//(isNaN(parseFloat(value))) ? value : parseFloat(value);
	},

	getNow: function(from, to, fx){			
	//(typeof(to) == "string" && typeof(from) == "string") ? from + to :		
...
Fx.Slide = Fx.Base.extend({
	...
	vertical: function(){
		///alert(this.element.offsetHeight +this.element.firstChild.scrollTop)
...
var XHR = new Class({
	...
	onSuccess: function(){
		//log.out("AJAX RESPONSE RECEIVED")
		this.response = {
...
Morph
	...
	taken from mootools demo 5.4.08
...
Fx.Morph = Fx.Styles.extend({
	...
	start: function(className){
		...
		// ?? this.setClass(className) // added so you can ask it its new class.
...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
NOTE, CHANGE THIS SECTION WITH EXTREME CAUTION AND CARE. ANY CHANGE HERE COULD BREAK IE,
AND BREAK ANY UI THAT USES IT INCLUDING REPORT WRITER. ALL CHANGES MUST BE TESTED EXHAUSTIVELY!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/ 

// "backgroundPosition","opacity","color","cursor",  // cursor breaks in ie. "","background", 
Fx.CSS.Styles = ["color", "opacity", "fontColor", "width", "height", "bottom", "right", "fontSize", "letterSpacing", "lineHeight", "textIndent"];
//"backgroundColor", 
//if(!window.ie6) Fx.CSS.Styles[Fx.CSS.Styles.length] = "backgroundColor";
